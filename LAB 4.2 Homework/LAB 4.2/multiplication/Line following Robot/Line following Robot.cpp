...
void controlMotor(int leftSensor, int rightSensor) 
{
    ...
    {
        cout << "Status: เดินหน้าตรง (Forward)" << endl;
    }
    
    else if (leftSensor == 1 && rightSensor == 0) 
    {
        cout << "Status: เลี้ยวซ้าย (Turn Left)" << endl;
    }
    
    else if (leftSensor == 0 && rightSensor == 1) 
    {
        cout << "Status: เลี้ยวขวา (Turn Right)" << endl;
    }
    
    else if (leftSensor == 0 && rightSensor == 0) 
    {
        cout << "Status: หยุด หรือเดินหน้าต่อ (Stop/Go)" << endl;
    }
}
